"""
Complete workflow script for processing a new meeting folder
and preparing it for web visualization.

This project supports two transcription backends:
1. Deepgram API - Fast cloud-based transcription with word-level confidence
2. Fine-tuned Whisper - Custom model trained on NPTEL Indian English dataset

Usage:
    python process_meeting.py <meeting_folder> [--use-whisper] [--model-info]
    
Example:
    python process_meeting.py projectmeeting1
    python process_meeting.py projectmeeting1 --use-whisper
    python process_meeting.py --model-info
"""
...
def main():
    ...
    step_desc = f"Step 1/3: Transcribing audio with {backend_name}"
    
    if has_transcripts:
        print(f"\n{'='*60}")
        print(step_desc)
        print(f"{'='*60}")
        print("✓ Transcripts already exist, skipping this step")
        step1 = True
    else:
        step1 = run_command(
        ...
        if not step1:
            print("\n❌ Transcription failed. Please check the error above.")
            sys.exit(1)
    
    # Step 2: Run phoneme-based pronunciation detection
    step2 = run_command(
        [sys.executable, "phoneme_pronunciation_detector.py", meeting_folder],
        "Step 2/3: Detecting pronunciation errors (phoneme comparison)"
    )
    
    if not step2:
        print("\n⚠ Pronunciation detection had issues, but continuing...")
    
    # Step 3: Update summary for web compatibility
    step3 = run_command(
        [sys.executable, "update_pronunciation_summary.py", meeting_folder],
        "Step 3/3: Preparing data for web visualization"
    )
    
    if not step3:
        print("\n❌ Summary update failed. Please check the error above.")
        sys.exit(1)
