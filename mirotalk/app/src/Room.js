...
module.exports = class Room {
    constructor(room_id, worker, io) {
        ...
        // Timeline Tracker for real-time event logging
        this.timelineTracker = null;
        const recordingsDir = config?.media?.recording?.dir || '../rec';
        const recordingsPath = path.isAbsolute(recordingsDir) 
            ? recordingsDir 
            : path.resolve(__dirname, recordingsDir);
        
        try {
            this.timelineTracker = new TimelineTracker(room_id, recordingsPath);
            log.info('Timeline tracker initialized for room', { room_id });
        } catch (error) {
            log.error('Failed to initialize timeline tracker', { 
                room_id, 
                error: error.message 
            });
        }
    ...
    async close() {
        // Finalize timeline before closing the room
        if (this.timelineTracker) {
            this.timelineTracker.logMeetingEnd('room_closed');
            this.timelineTracker.finalize();
            log.info('Timeline finalized for room', { room_id: this.id });
        }
    ...
    addPeer(peer) {
        ...
        // Log user join to timeline
        if (this.timelineTracker) {
            const peerInfo = peer.peer_info || {};
            this.timelineTracker.logUserJoin(peer.id, peer.peer_name, {
                peer_uuid: peerInfo.peer_uuid,
                os_name: peerInfo.os_name,
                browser_name: peerInfo.browser_name,
            });
        }
    }
    ...
    removePeer(socket_id) {
        ...
        // Log user leave to timeline before removing peer
        if (this.timelineTracker && peer) {
            this.timelineTracker.logUserLeave(socket_id, peer.peer_name, 'peer_removed');
        }
    ...
    logRecordingStart(socket_id, peer_name, producer_id, audio_file = null) {
        if (this.timelineTracker) {
            this.timelineTracker.logRecordingStart(socket_id, peer_name, producer_id, audio_file);
        }
    }

    logRecordingStop(socket_id, peer_name, producer_id, audio_file = null) {
        if (this.timelineTracker) {
            this.timelineTracker.logRecordingStop(socket_id, peer_name, producer_id, audio_file);
        }
    }
