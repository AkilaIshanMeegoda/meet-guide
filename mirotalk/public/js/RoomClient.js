...
class RoomClient {
    constructor(
        ...
        this.perParticipantRecorder = null;
        this.usePerParticipantRecording = true; // Enable per-participant recording
    ...
    async handleRoomInfo(room) {
        ...
        for (let peer of Array.from(this.peers.keys()).filter((id) => id == this.peer_id)) {
            ...
            if (room.recording && room.recording.recSyncServerRecording) {
                ...
                // Keep server's recording configuration - server decides if recording is enabled
                // Note: recSyncServerRecording stays true as set by server
        ...
        for (let peer of Array.from(this.peers.keys()).filter((id) => id !== this.peer_id)) {
            ...
            // Silent recording - don't show recording status
            // if (peer_recording) {
    ...
    async startLocalMedia() {
        ...
        // Debug: Check recording configuration
        console.log('11.1 ----> Recording Config Check', {
            usePerParticipantRecording: this.usePerParticipantRecording,
            recording: this.recording,
            recSyncServerRecording: this.recording?.recSyncServerRecording
        });
        
        // Auto-start per-participant recording if enabled
        if (this.usePerParticipantRecording && this.recording?.recSyncServerRecording) {
            await this.sleep(1000); // Wait for audio streams to stabilize
            this.startPerParticipantRecording();
            console.log('12 ----> AUTO-START PER-PARTICIPANT RECORDING');
        } else {
            console.warn('11.2 ----> Recording NOT started:', {
                reason: !this.usePerParticipantRecording ? 'usePerParticipantRecording is false' :
                        !this.recording?.recSyncServerRecording ? 'recSyncServerRecording is false' :
                        'unknown'
            });
        }
    }
    ...
    startRecording() {
        // Use per-participant recording if enabled
        if (this.usePerParticipantRecording) {
            return this.startPerParticipantRecording();
        }
        
        // Original mixed recording code
        recordedBlobs = [];
    ...
    startPerParticipantRecording() {
        console.log('[RoomClient] Starting per-participant recording...');
        
        try {
            // Initialize per-participant recorder if not exists
            if (!this.perParticipantRecorder) {
                this.perParticipantRecorder = new PerParticipantRecorder(this);
            }
            
            // Start recording all participants
            this.perParticipantRecorder.startRecording();
            
            this._isRecording = true;
            this.event(_EVENTS.startRec);
            this.recordingAction(enums.recording.start);
            // this.sound('recStart'); // Silent recording - no sound
            
            // this.userLog('success', '🎙️ Per-participant recording started', 'top-end', 3000); // Silent recording - no notification
            console.log('[RoomClient] Per-participant recording started successfully (silent mode)');
            
        } catch (err) {
            this.handleRecordingError('Exception while starting per-participant recording: ' + err);
        }
    }
    ...
    stopRecording() {
        // Use per-participant recording if enabled
        if (this.usePerParticipantRecording && this.perParticipantRecorder) {
            this._isRecording = false;
            this.perParticipantRecorder.stopRecording();
            this.event(_EVENTS.stopRec);
            this.recordingAction(enums.recording.stop);
            // this.sound('recStop'); // Silent recording - no sound
            // this.userLog('success', '⏹️ Per-participant recording stopped', 'top-end', 3000); // Silent recording - no notification
            return;
        }
    ...
    recordingAction(action) {
        // Silent recording - do not broadcast recording actions to other participants
        ...
        //     action: action,
        // });
        console.log('[Silent Recording] Recording action:', action);
    }

    handleRecordingAction(data) {
        console.log('[Silent Recording] Suppressed recording action', data);
        // Suppress all recording notifications and messages
