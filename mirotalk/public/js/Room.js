...
// HTTPS redirect disabled for local development (iframe embedding)
...
function roomIsReady() {
    ...
    // Silent recording mode - hide all recording buttons
        ...
        document.addEventListener('keydown', (event) => {
            ...
            switch (key) {
                ...
                case 'x':
                    ...
                    break;
                case 'r':
                    // Silent recording mode - disable keyboard shortcut
                    ...
                    // }
                    // isRecording ? stopRecButton.click() : startRecButton.click();
                    console.log('[Silent Recording] Recording keyboard shortcut disabled');
                    break;
                case 'j':
...
function handleRoomClientEvents() {
    rc.on(RoomClient.EVENTS.startRec, () => {
        console.log('[Silent Recording] Client start recording');
        // hide(startRecButton); // Keep hidden
        // show(stopRecButton); // Keep hidden
        // show(pauseRecButton); // Keep hidden
        // show(recordingTime); // Keep hidden
        // startRecordingTimer(); // No visible timer
        isRecording = true;
        // rc.updatePeerInfo(peer_name, socket.id, 'recording', true); // Don't show recording status to others
    });
    ...
    rc.on(RoomClient.EVENTS.stopRec, () => {
        console.log('[Silent Recording] Client stop recording');
        // hide(stopRecButton); // Already hidden
        // hide(pauseRecButton); // Already hidden
        // hide(resumeRecButton); // Already hidden
        // hide(recordingTime); // Already hidden
        // show(startRecButton); // Keep hidden
        // stopRecordingTimer(); // No visible timer
        isRecording = false;
        // rc.updatePeerInfo(peer_name, socket.id, 'recording', false); // Don't show recording status to others
    });
...
async function leaveRoom(allowCancel = true) {
    // Silent recording mode - skip recording popup and leave directly
    // if (rc.isRecording() || recordingStatus.innerText != '0s') {
    //     recShowInfo = false;
    ...
    // Silently save recording if active
    if (rc.isRecording() || recordingStatus.innerText != '0s') {
        recShowInfo = false;
        rc.saveRecording('User is leaving the room, saving recording before exit');
    }
