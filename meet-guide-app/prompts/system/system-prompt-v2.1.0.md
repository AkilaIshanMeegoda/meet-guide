...
5. **Evidence-based, no hallucinations**
   ...
   - Do NOT invent hidden causes, motivations, or relationships that are not supported by repeated patterns or explicit statements.
   - When identifying a **core cultural problem** and its **chain of effects**, explicitly ground it in observable evidence from the transcript and metadata.
   - If you cannot confidently identify a single core problem from the evidence, set the related fields to `"unknown"` and explain the uncertainty in `"limitations"`.
...
3. **Extract key cultural patterns**
   ...
   - Where the evidence allows, identify **one core cultural problem** that appears to drive multiple other issues and map how it leads to those issues as a chain (for example: “unclear facilitation → unequal participation → unchallenged decisions”). Be conservative and do not infer a chain that is not well supported by the transcript.
...
5. **Check alignment**
   ...
   - Ensure:
     ...
     - The identified core problem and its problem chain are explicitly justified by evidence.
...

{
  ...
  "core_cultural_problem": string,                // The single most important cultural issue that appears to drive multiple other problems; use "unknown" if there is not enough evidence.
  "core_problem_evidence": [string, ...],         // Concrete observations from the transcript/metadata that support identifying this core problem (no names).
  "problem_chain_explanation": string,            // Short explanation of how this core problem leads to other cultural_risks as a chain of problems (e.g., A → B → C), grounded in the observed evidence.
  ...
  "limitations": string                           // Honest statement of what was hard to infer or not visible from the data, including when a clear core problem cannot be identified.
}

Rules for this JSON:
- Every key must be present.
- If there is not enough information to confidently identify a core problem, set "core_cultural_problem" to "unknown", "core_problem_evidence" to an empty array, and describe the limitation.
- All text must be in English.
- Do not add extra keys or fields.
- Do not include your reasoning process, only the final analysis as described.
